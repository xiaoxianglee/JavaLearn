...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    public void execute(Runnable command) {
        ...
        if (poolSize >= corePoolSize || !addIfUnderCorePoolSize(command)) {
			//xiaoxiang：对于SynchronousQueue，它的offer()在无法插入时返回false
			//注意：这里只能使用offer，而不能使用put，因为SynchronousQueue.put()在失败时会阻塞
            if (runState == RUNNING && workQueue.offer(command)) {  //xiaoxiang：在加入到queue中的时候，runState为RUNNING。如果是SynchronousQueue，刚开始的时候，thread pool为空，没worker thread。workQueue.offer()将会被阻塞，直到有workerThread从queue中取出task。但是，此时，主线程已经被阻塞，无法创建workerThread啊？
                if (runState != RUNNING || poolSize == 0)  //xiaoxiang：再加入之后，runState变成了非RUNNING
    ...
    private void ensureQueuedTaskHandled(Runnable command) {
        ...
        try {
            int state = runState;
            if (state != RUNNING && workQueue.remove(command))  //xiaoxiang：如果task被queue的同时，调用了shutdownNow，而且，task还出现在queue中，那么reject整个task
                reject = true;
            else if (state < STOP &&
                     poolSize < Math.max(corePoolSize, 1) &&
                     !workQueue.isEmpty())
                t = addThread(null);  //xiaoxiang：保证至少有一个live thread来执行这个task
        } finally {
            mainLock.unlock();
        }
        if (reject)
        ...
        for (;;) {
            try {
                ...
                if (state > SHUTDOWN)
                    return null;
                Runnable r;
				//xiaoxiang：已经调用过了shutdown()，把ThraedPoolExecutor.state是为了SHUTDOWN状态。将不再接收新任务，但是worker还是会执行queue中的task
                if (state == SHUTDOWN)  // Help drain queue   
                    r = workQueue.poll();
					
				//xiaoxiang：对于过多的worker thread(超过corePoolSize的thread)，或者允许core thread受keepAliveTime限制
                else if (poolSize > corePoolSize || allowCoreThreadTimeOut)
                    r = workQueue.poll(keepAliveTime, TimeUnit.NANOSECONDS);  //对于非core thread，如果keepAliveTime时间内没有拿到task
                else
                    r = workQueue.take();  //对于core thread，默认情况下，core thread会一直等待
                if (r != null)
                    return r;
                if (workerCanExit()) {  //xiaoxiang：没有拿到task，r==null
                    if (runState >= SHUTDOWN) // Wake up others
                        interruptIdleWorkers();  //r=null的时候，如果state>=SHUTDOWN，那么interrupt每个thread
                    return null;   //否则，等待worker thread自己结束
                }
                // Else retry
            } catch (InterruptedException ie) {
    ...
    static final int STOP       = 2;
    static final int TERMINATED = 3;
	shutdown()修改state为SHUTDOWN，满足state<STOP。如果此时workQueue不为空，那么把state改回为RUNNING
			*/
            if (state < STOP && !workQueue.isEmpty()) {  
                state = RUNNING; // disable termination check below
                addThread(null);  //创建一个worker thread，它的第一个任务为null
            }
            if (state == STOP || state == SHUTDOWN) {  //xiaoxiang：如果state之前为SHUTDOWN,并且workQueue为空；或者，state之前为STOP，那么直接terminate整个pool
                runState = TERMINATED;
    ...
    public void shutdown() {
		 ...
		 * shutdown只是把runState改为SHUTDOWN了，然后interrupt所有的
		 * 空闲的worker thread(阻塞在getTask()中的worker thread)
		 * if(没有task和thread了) 直接terminate整个pool
		 * else 最后一个worker关闭lights
         *
	...
	if (security != null)
        ...
        try {
            ...
            int state = runState;  
            if (state < SHUTDOWN)
                runState = SHUTDOWN;  //修改state为SHUTDOWN

            try {
                for (Worker w : workers) {
                    w.interruptIfIdle();  //尝试interrupt那些idle的worker thread
                }
            ...
            tryTerminate(); // Terminate now if pool and queue empty  如果没有thread和task了，那么terminate整个pool
        } finally {
            mainLock.unlock();
        }
    }
	 ...
	 * shutdownNow调用interrupt来cancel task。如果给thread不响应interrupt，那么shutdownNow也没有效果
     *
	...
	if (security != null)
        ...
        try {
            ...
            int state = runState;
            if (state < STOP)
                runState = STOP;  //1，把state设为STOP

            try {
                for (Worker w : workers) {  //2，对所有的worker thread，调用interrupt
                    w.interruptNow();
                }
            } catch (SecurityException se) { // Try to back out
                runState = state;
            ...
            List<Runnable> tasks = drainQueue();  //先清空workerQueue
            tryTerminate(); // Terminate now if pool and queue empty  //然后，tryTerminate()将终止pool
            return tasks;
        } finally {
            mainLock.unlock();
        }
    ...
    public void allowCoreThreadTimeOut(boolean value) {
        if (value && keepAliveTime <= 0)   //在调用allowCoreThreadTimeOut的时候，如果keepAliveTime=0，那么抛出异常
    ...
    public void setKeepAliveTime(long time, TimeUnit unit) {
        if (time < 0)
            throw new IllegalArgumentException();
        if (time == 0 && allowsCoreThreadTimeOut())   //在setKEepAliveTime()的时候，如果keepAliveTime=0且allowCoreThreadTimeout，那么抛出异常
            throw new IllegalArgumentException("Core threads must have nonzero keep alive times");
