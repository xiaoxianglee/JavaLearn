...
//jdk1.6.0_43
/*
所有的ctor，都会为底层数组新建一个copy
toArray，使用保护性拷贝，返回一个底层数组的拷贝

使用底层数组来保存对象，底层数组是volatile和transisent的

add，remove会使用内部锁；copy底层数组，修改copy，并使得array执行copy
get不会加锁

iterator()和listiterator()返回COWIterator，它不支持add，set，remove操作。在创建COWIterator的时候，会保存原始底层数组elements的一个snapshot。在COWIterator中的所有操作都是在snapshot上进行的。

和ConcurrentHashMap不同，addAll会是atomic的，因为使用了内部锁

    public <T> T[] toArray(T a[]) {
        Object[] elements = getArray();
        int len = elements.length;
        if (a.length < len)
	    return (T[]) Arrays.copyOf(elements, len, a.getClass());  //如果参数a[]的长度不够，那么新建一个底层数组copy
	else {
	    System.arraycopy(elements, 0, a, 0, len);   //否则，把底层数组拷贝到参数a[]中，并把a[len]=null
	    if (a.length > len)
		a[len] = null;
	    return a;
	}
...
public class CopyOnWriteArrayList<E>
    ...
    transient final ReentrantLock lock = new ReentrantLock();  //使用内部锁：private lock

    /** The array, accessed only via getArray/setArray. */
    private volatile transient Object[] array;   //使用底层数组来保存对象，底层数组是volatile和transisent的
    ...
    public CopyOnWriteArrayList() {
        setArray(new Object[0]);  //default cotr，新建一个长度为0的数组
    }
     ...
     */
	//问题：如果类型相同，那么CopyOnWriteArrayList会和c用同一个数组吗？不会。因为Collection.toArray()会返回一个新数组。比如：ArrayList.toArray()
	/*
	...
	*/
	...
	if (elements.getClass() != Object[].class)  //如果数组元素类型不同，那么新建一个数组copy
     ...
     */
    public Object[] toArray() {   //xiaoxiang: 保护性拷贝，defensive copy：返回一个底层数组的copy
        Object[] elements = getArray();
	...
	try {
	    ...
	    if (oldValue != element) {
		int len = elements.length;
		Object[] newElements = Arrays.copyOf(elements, len);  //赋值数组
		newElements[index] = element;  //修改拷贝
		setArray(newElements);   //修改底层数组的reference
	    } else {
