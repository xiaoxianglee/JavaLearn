...
public abstract class ClassLoader {
     ...
     *   has already been loaded.  </p></li> 调用findLoadedClass(String)检查class是否已经被load过了
     *
     *   <li><p> Invoke the {@link #loadClass(String) <tt>loadClass</tt>} method
     *   on the parent class loader.  If the parent is <tt>null</tt> the class
     *   loader built-in to the virtual machine is used, instead.  </p></li>
	 *   调用parant class loader的loadClass(String)方法。
     *
     *   <li><p> Invoke the {@link #findClass(String)} method to find the
     *   class.  </p></li>调用findClass(String)来找class
     *
     * </ol>
     *
	 ...
	 * 如果resolve==true，那么调用resolveClass(Class)
     *
     * <p> Subclasses of <tt>ClassLoader</tt> are encouraged to override {@link
     * #findClass(String)}, rather than this method.  </p>
	 * 子类最好覆盖findClass而不是loadClass
     *
     * @param  name
     *         The <a href="#name">binary name</a> of the class
     *
	...
	Class c = findLoadedClass(name);  //检查class是否已经被load过了
	if (c == null) {
	    try {
		if (parent != null) {  //调用parent的loadClass
		    c = parent.loadClass(name, false);
		} else {
		    c = findBootstrapClassOrNull(name);  //如果没有parent，那么调用bootstrap class loader。这里也就说明了，extenstion class loader的父类到底是null还是bootstrap class loader
		}
	        ...
	        // to find the class.
	        c = findClass(name);  //如果父类没有load成功class，那么调用findClass
	    }
	}
	if (resolve) {  //如果resolve=true，那么调用resolveClass
	    resolveClass(c);
	}
	return c;
    }
    ...
    */
	...
	throws ClassNotFoundException
    {
	ClassLoader system = getSystemClassLoader();  //最终获取Launcher.loader,即：app classloader
	if (system == null) {  //如果system classloader为null
	    if (!checkName(name))
		throw new ClassNotFoundException(name);
	...
	return system.loadClass(name);  //用system classloader加载类
    }
...

	 ...
	 * 返回system class loader。这是ClassLoader实例的默认parent，也是start application的class loader
     *
	...
	if (!sclSet) {
	    ...
	    if (l != null) {
		Throwable oops = null;
		scl = l.getClassLoader();  //xiaoxiang：返回Launcher.loader. Launcher的ctor中：创建ext classloader，创建app classloader，把app classloader设为thread.contextClassLoader。而Launcher.getClassLoader()返回app class loader
	        try {
